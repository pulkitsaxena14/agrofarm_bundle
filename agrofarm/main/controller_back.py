...
def view_question(q_id):
    ...
    ###q_user = Users.query.filter_by(id=question.u_id).first()

    answer_accepted = Answers.query.filter_by(q_id=q_id, accepted="YES").first()
    ###all_answers = Answers.query.filter_by(q_id=q_id, accepted="NO").all()
    ...
    #    print data
    #all_answers = db.session.execute("SELECT * FROM ANSWERS WHERE ANSWERS.q_id= :val", {'val' : ques_id})
    #for data in all_answers:
...
def submit_answer(q_id):
    ...
    try:
        ...
        return jsonify(response), 500
    #response = jsonify(output)
#result['answers'] = Answers.query.filter_by(u_id=session['u_id'])
@main.route('help')
def help():
...
def vote():
    ...
    if entity_type == "QUESTION":
        ...
        print ("VOTING QUESTION <U_id: {}, Q_id: {}>\n", format(u_id, entity_id))
        if result is None:
            ...
            try:
                ...
                if vote == "UP":
                    print "Adding Upvote to Questions table...\n"
                    update_questions.upvotes += 1
                else:
                    print "Adding Downvote to Questions table...\n"
        ...
        if result is None:
            ...
            try:
                ...
                if vote == "UP":
                    print "Adding Upvote to Answers table...\n"
                    update_answers.upvotes += 1
                else:
                    print "Adding Downvote to auestions table...\n"
...
def flash_test():
    flash("TESTING!!")
    return render_template("generic_test.html")
...
#        current_app.logger.info('Adding a new tester %s: %s.', (name, email))
...
#            current_app.logger.error(e)
#
#            return redirect(url_for('main.submit_user'))
#
#        return redirect(url_for('main.index'))
#
...
#@main.route('testers')
#def display_testers():
#    testers = [tester for tester in Login.query.all()]
#    current_app.logger.info('Displaying all Testers.')
#    return render_template("testers.html", testers=testers)
#
#@main.route('ques_test')
#def display_quest():
#    testers = [tester for tester in Questions.query.order_by(Questions.asked.desc()).all()]
#    current_app.logger.info('Displaying all Testers.')
#    return render_template("ques_test.html", testers=testers)
